/* Pete's farewell book */

function updateDepth(book, newPage) {
  var page = book.turn("page"),
    pages = book.turn("pages"),
    depthWidth = 14 * Math.min(1, (page * 2) / pages);

  newPage = newPage || page;

  if (newPage > 3)
    $(".fp-book .p2 .depth").css({
      width: depthWidth,
      left: 20 - depthWidth,
    });
  else $(".fp-book .p2 .depth").css({ width: 0 });

  depthWidth = 14 * Math.min(1, ((pages - page) * 2) / pages);

  if (newPage < pages - 3)
    $(".fp-book .p17 .depth").css({
      width: depthWidth,
      right: 20 - depthWidth,
    });
  else $(".fp-book .p17 .depth").css({ width: 0 });
}

function loadPage(page) {
  $.ajax({ url: "pages/page" + page + ".html" }).done(function (pageHtml) {
    $(".fp-book .p" + page).html(
      pageHtml.replace("samples/farewell-pete/", "")
    );
  });
}

function addPage(page, book) {
  console.log("called addpage. page: ", page);
  var id,
    pages = book.turn("pages");
...
function zoomHandle(e) {
  if ($(".fp-book").data().zoomIn) zoomOut();
...
function zoomThis(pic) {
  ...
  $(".fp-book").data().zoomIn = true;

  $(".fp-book").turn("disable", true);
...
function zoomOut() {
  var transitionEnd = $.cssTransitionEnd(),
    completeTransition = function (e) {
      $("#book-zoom").unbind(transitionEnd);
      $(".fp-book").turn("disable", false);
      $("body").css({ overflow: "auto" });
      moveBar(false);
    };

  $(".fp-book").data().zoomIn = false;
